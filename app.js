...
/* Student names, grades and special requests are typed by the person using
   the page and then written into markup. Escaped so a stray angle bracket or
   quote cannot break the layout or inject anything. */
...
/* Dates and the ordering cutoff follow the school's clock, not the device's,
   so a parent whose phone is set to another timezone sees the same calendar
   the checkout function enforces. */
...
/* The deadline sets the price, not whether ordering is possible.
   Each meal separately stops being orderable at orderCutoffHour,
   orderCutoffDaysBefore days before it is served. 0 and 9 means 9 am that day. */
const cutoffPassed = (dayDate) => {
  const s = state.data.settings;
  const daysBefore = s.orderCutoffDaysBefore == null ? 0 : s.orderCutoffDaysBefore;
  const hour = s.orderCutoffHour == null ? 9 : s.orderCutoffHour;
  const lastDay = shiftDays(dayDate, -daysBefore);
  const now = schoolNow();
  return now.date > lastDay || (now.date === lastDay && now.hour >= hour);
};

/* Master switch. Set orderingOpen to false in menus.json to show the menu but
   take no money: nothing is orderable and the checkout block is replaced. */
const orderingOpen = () => state.data.settings.orderingOpen !== false;

/* One date in menus.json drives every place the opening is mentioned, so it
   can never say two different things. */
const opensTitle = () => {
  const s = state.data.settings;
  if (s.opensOn) {
    return 'Ordering opens ' + parseDate(s.opensOn)
      .toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' });
  }
  return s.closedTitle || 'Ordering opens soon';
};
...
const canOrder = (month, day) =>
  orderingOpen() && month.published && !cutoffPassed(day.date);
...
function applyOrderingState() {
  const s = state.data.settings;
  const box = document.getElementById('pausedBox');
  const payTitle = document.getElementById('stepPay');
  if (!box) return;
  ...
  box.hidden = false;
  document.getElementById('pausedTitle').textContent = opensTitle();
  document.getElementById('pausedMessage').textContent = s.closedMessage || '';
  if (payTitle) {
    payTitle.innerHTML =
      '<span class="stepnum">3</span> ' + opensTitle() +
      '<span class="hint">Leave your email and we will tell you</span>';
  }
  wireNotifyForm();
}
...
function wireNotifyForm() {
  ...
  form.onsubmit = async (e) => {
    ...
    /* Written to two places on purpose. Netlify Forms needs form detection
       switched on in the dashboard, and the spreadsheet needs Google
       credentials. Either one alone can be misconfigured without anyone
       noticing, and a lost list cannot be rebuilt. */
    const toNetlify = window.PREVIEW ? Promise.resolve(true) : fetch('/', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body
    }).then((r) => r.ok).catch(() => false);

    const toSheet = window.PREVIEW ? Promise.resolve(true) : fetch('/.netlify/functions/notify', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, 'bot-field': form['bot-field'].value })
    }).then((r) => r.ok).catch(() => false);

    const [netlifyOk, sheetOk] = await Promise.all([toNetlify, toSheet]);

    if (netlifyOk || sheetOk) {
      form.hidden = true;
      done.textContent = 'Thank you. We will email you the moment ordering opens.';
    } else {
      done.textContent =
        'That did not send. Please email ' + (state.data.settings.contactEmail || 'us') + ' and we will add you.';
    }
  };
}
...
function renderStudents() {
  ...
  if (!hasStudent && orderingOpen()) form.classList.add('open');

  const banner = document.getElementById('needsStudent');
  if (banner) banner.hidden = hasStudent || !orderingOpen();

  const step1 = document.getElementById('stepStudents');
  if (step1) step1.hidden = !orderingOpen();
...
function renderMonth() {
  ...
  if (!orderingOpen()) {
    ...
    notice.textContent = opensTitle();
  } else if (!monthHasOpenDays(month)) {
    notice.className = 'deadline closed';
...
function ticket(day, month) {
  ...
  if (isPast) {
    ...
    btn.disabled = true;
    btn.textContent = 'Ordering opens soon';
...
function openDialog(day) {
  ...
  roster.forEach(({ s, i }) => {
    ...
    row.innerHTML = `
      ...
      <label for="who_${i}">${esc(s.name)}${s.grade ? ', ' + esc(s.grade) : ''}${
...
function renderCart() {
  ...
  state.students.forEach((student, si) => {
    ...
    group.innerHTML = `<h3>${esc(student.name)}${student.grade ? ', ' + esc(student.grade) : ''}
      ...
      .forEach((item) => {
        ...
        row.innerHTML = `
          ...
          <span class="what">${item.meal}
            ...
            ${item.note ? `<span class="extras">Request: ${esc(item.note)}</span>` : ''}
          </span>
...
async function checkout(button) {
  ...
  try {
    ...
    err.innerHTML = `<p class="error">${esc(e.message)} Try again, or email ${esc(state.data.settings.contactEmail)}.</p>`;

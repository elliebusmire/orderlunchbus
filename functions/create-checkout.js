...
// Current hour at the school, 0 to 23.
const hourAtSchool = () => Number(new Intl.DateTimeFormat('en-US', {
  timeZone: SCHOOL_TZ, hour: 'numeric', hourCycle: 'h23'
}).format(new Date()));
...
// Date arithmetic on YYYY-MM-DD strings, done in UTC so the server's own
// timezone can never shift a date by one.
const shiftDays = (iso, n) => {
  const [y, m, d] = iso.split('-').map(Number);
  return new Date(Date.UTC(y, m - 1, d + n)).toISOString().slice(0, 10);
};

const hourLabel = (h) => (h === 0 ? '12 am' : h === 12 ? 'noon' : h < 12 ? `${h} am` : `${h - 12} pm`);

/* Ordering for a meal closes at orderCutoffHour, orderCutoffDaysBefore days
   before it is served, in school time. 0 and 9 means 9 am on the day. */
function cutoffPassed(serviceDate, s) {
  ...
  const lastDay = shiftDays(serviceDate, -daysBefore);
  const today = todayISO();
  return today > lastDay || (today === lastDay && hourAtSchool() >= hour);
}

function cutoffPhrase(s) {
  const daysBefore = s.orderCutoffDaysBefore == null ? 0 : s.orderCutoffDaysBefore;
  const at = hourLabel(s.orderCutoffHour == null ? 9 : s.orderCutoffHour);
  if (daysBefore === 0) return `at ${at} on the day it is served`;
  if (daysBefore === 1) return `at ${at} the day before it is served`;
  return `at ${at}, ${daysBefore} days before it is served`;
}
...
exports.handler = async (event) => {
  ...
  // Lowercased so the parent portal can find every order under one address.
  const parentEmail = clean(parent.email, LIMITS.email).toLowerCase();
  ...
  for (const item of items) {
    ...
    if (typeof item.date !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(item.date)) {
      return { statusCode: 400, body: JSON.stringify({ error: 'One of the meals has a date we cannot read.' }) };
    }
    if (cutoffPassed(item.date, s)) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: `It is too late to order the meal on ${new Date(item.date + 'T12:00:00Z').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', timeZone: 'UTC' })}. Each meal closes ${cutoffPhrase(s)}. Remove it and try again.` })
      };
    }
  ...
  const metadata = {
    // Tags the payment as a Lunch Bus order. The webhook and the parent
    // portal only act on sessions carrying this tag.
